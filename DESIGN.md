...
| 卡片 2 | `--card-2` | `#ffffff` | 工具页行内卡背景（`common.css` 中已统一为透明，个别视图保留） |
| 分割线 | `--rule` | `#e3e0d9` | 主分割线（侧栏右边框、视图边框） |
| 次分割 | `--rule-soft` | `#ecebe6` | 次要分割线 |
...
**配色系统（6 色）**：通过 `[data-color]` 选择器覆盖 `--aside-top`/`--aside-2`/`--aside-3`/`--border-accent`/`--link`/`--accent` 六项变量。浅色与深色模式各 6 套完整值。

| 配色 | 默认(蓝) | 浅紫 | 绿 | 玫瑰 | 青 | 暖色 |
|---|---|---|---|---|---|
| `--aside-top` | `#f3f6fd` | `#f3eefb` | `#eef5ed` | `#fdf0f0` | `#edf5f3` | `#f5efe6` |
| `--aside-2` | `#dce8f9` | `#ece4f7` | `#dbe8d8` | `#f5dcdc` | `#d4e8e4` | `#e8decc` |
| `--aside-3` | `#c8d8f0` | `#dcd0ee` | `#c8d8c4` | `#ecc8c8` | `#bcd8d4` | `#d9ccb5` |
| `--border-accent` | `#c4d4eb` | `#d4c8ec` | `#c8d4c4` | `#ecc8c8` | `#bcd4d0` | `#d0c4b0` |
| `--accent` | `#5b8cff` | `#8b6cf0` | `#5cb86c` | `#d96a6a` | `#5cb8b8` | `#d4a040` |
...
│ background │        text-area × 2 (flex-1 平分剩余空间)        │
│ linear      │                                                │
│ gradient    │     工具页通用模板：                             │
│ (180deg,    │       header.page-head (H1)                    │
│  var(--aside├─ .section-title: "配置" / "输入" / "输出"      │
│   -top),    │       .config (flex-col gap-4px) ← 可选的配置区  │
│  var(--aside│       text-area (flex-1, 输入)                   │
│  ))         │       text-area (flex-1, 输出 readonly)          │
└────────────┴─────────────────────────────────────────────────┘

最小窗口  880 × 600     默认窗口  1100 × 760
```

**自适应**：宽度由 `[280px 1fr]` 网格驱动；高度由 `flex-col flex-1` 链传递至 `<router-view>` 内的所有 flex 容器，最终由两个 `<textarea class="text-area">` 平分主区剩余空间。`common.css` 中 `.text-area` 的 `min-height: 200px` 确保最小高度，`flex: 1` 平分剩余空间。图像处理视图使用 `position: relative` + 进度条覆盖。
...
- 内嵌 SVG 统一 `14 × 14px`。
- 内嵌 SVG 统一定义在 `common.css` 的 `.row-icon svg { width: 18px; height: 18px; }`。
...
- `border: 1px solid var(--border-accent)`、圆角 `var(--r-md)`、`background: transparent`。
- `padding: 12px 14px`、字体 `var(--mono) 13.5px`、`line-height: 1.6`。
- `flex: 1; min-height: 200px`，`resize: none; outline: none`。
- `:focus` 时 `border-color: var(--aside-3)`。
- `color: var(--ink)`，`caret-color: var(--ink)`。
- `word-break: break-all` 确保长无分隔文本换行。
- `readonly` 输出区域使用 `<textarea readonly>` 代替原 `<pre>`（与 `common.css` 样式一致）。

**`.input`（单行输入框）**：

- `border: 1px solid var(--border-accent)`、圆角 `var(--r-md)`、`background: transparent`。
- `padding: 7px 12px`、`font-size: 13px`。
- `:focus` 时 `border-color: var(--link)`。
- `:disabled` 时 `opacity: 0.5`。
- 视图特有宽度通过 scoped CSS 覆盖（如 `.input { flex: 1 }` 或 `min-width: 220px`）。
...
`.config`（`common.css`）：
- 背景 `transparent`、圆角 `var(--r-md)`、`padding: 6px`、`flex-col gap: 4px`。
- 所有视图统一，不再按视图区分背景色。

`.row`（`common.css`）：
- 网格 `[44px 1fr auto]`、`gap: 12px`、`min-height: 64px`、`padding: 14px 16px`、圆角 `8px`、`box-shadow: 0 1px 0 rgba(0,0,0,0.02)`。
- 背景 `transparent`、`border: 1px solid var(--border-accent)`。
...
- 单输入 → 单输出，**输出区只读**（`<textarea readonly class="text-area">`），用户不可编辑。
...
3. 自定义元件（Switch / Pill / NavItem 等）保留 scoped CSS；公用控件优先用 `common.css` 的 class（`.btn` / `.input` / `.text-area` 等）。
