...
本地开发者工具集（DevToys 风格）桌面应用：左侧搜索导航 + 右侧工具页。基于 **Wails v2（Go 后端 + Vue 3 前端）** 构建，支持浅色/深色主题 + 5 色配色系统。
...
| ⚙️ 设置 | 主题切换（浅色/深色）+ 5 色配色（蓝/紫/绿/玫瑰/青） | 纯前端 |
...
- **主题与配色**由 `stores/theme.ts` 管理，`<html data-theme>` + `<html data-color>` 切换，`tokens.css` 用 `[data-theme='dark']` 覆盖颜色变量 + `[data-color='xxx']` 提供 5 套配色。Naive UI 在 `App.vue` 按主题传 `darkTheme`。
